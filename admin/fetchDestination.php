...
if ($search != '') {
    $sql = "SELECT * FROM destination WHERE location LIKE '%$search%' OR days LIKE '%$search%' OR price LIKE '%$search%'";
} else {
...
if (mysqli_num_rows($res) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        ...
        echo '</td>
                <td>' . $row['rate'] . '</td>
                <td><a href="addDestination.php?u_id=' . $row['id'] . '" class="btn edit-btn">Edit</a></td>
                <td><a href="viewDestination.php?d_id=' . $row['id'] . '" class="btn delete-btn" onclick="return confirm(\'Are you sure to delete this destination?\');">Delete</a></td>
              </tr>';
    }
